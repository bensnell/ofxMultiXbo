...
void ofApp::setup(){
	...
	ofxSuperLog::getLogger()->setWindowsEventLogging(true, "BasicSketch2");

	ofLogNotice("This app") << "Is running away";


	fbo.clear(); fbo.allocate(100, 100, GL_LUMINANCE);
	//fbo.clear(); fbo.allocate(100, 100, GL_RGB);
	//fbo.clear(); fbo.allocate(100, 100, GL_RGBA);
...
void ofApp::draw(){
	...
	//cout << GL_LUMINANCE << endl;
	cout << fbo.getTexture().texData.glInternalFormat << endl;
